  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-red-50 to-pink-50 p-5">
      <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl text-center">
        ...
        <button
          onClick={onRetry}
          className="w-full bg-blue-500 text-white font-bold rounded-2xl hover:shadow-lg transform hover:-translate-y-1 transition-all duration-200"
        >
          Coba Lagi
        </button>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-indigo-100 p-4 flex items-center justify-center">
      <div className="bg-white rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-blue-700 mb-2">Hasil Quiz</h1>
...
export default function RandomQuizPage() {
  ...
  const playLetterAudio = (
    ...
    return new Promise((resolve, reject) => {
      if (!audioUrl) {
        reject(new Error(`Audio tidak tersedia untuk huruf ${letter}`));
        return;
      }
      ...
      try {
        ...
        audio.onerror = () => {
          setIsPlaying(false);
          setPlayingOption(null);
          reject(new Error(`Gagal memutar audio untuk huruf ${letter}`));
        };
          ...
          <div className="flex justify-between gap-4">
            ...
            <button
              ...
              className={`flex-1 py-3 rounded-2xl font-bold transition-all ${
                currentIndex === totalQuestions - 1 || isPlayingFeedback
                  ? "bg-gray-200 text-gray-400 cursor-not-allowed"
                  : "bg-purple-500 to-pink-600 text-white hover:shadow-lg transform hover:-translate-y-1"
              }`}
            >
              Selanjutnya ▶
            </button>
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl font-bold text-lg transition-all ${
                isPlayingFeedback || isUpdatingScore
                  ...
                  : "bg-gradient-to-r from-red-500 to-orange-600 text-white hover:shadow-xl transform hover:-translate-y-1"
              }`}
            >
              {isUpdatingScore ? (
