    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-gray-50 to-blue-50 p-5">
      <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl text-center">
        ...
        <p className="text-gray-600 mb-6">
          Belum ada soal quiz kata yang tersedia.
        </p>
        <button
          onClick={onRetry}
...
export default function RandomWordQuizPage() {
  ...
  const fetchRandomQuizzes = async () => {
    try {
      ...
      // PERUBAHAN: Fetch untuk kata
      const response = await fetch("/api/quiz/kata/random", {
        cache: "no-store",
      });
  ...
  const playWordAudio = (
    ...
    return new Promise((resolve, reject) => {
      if (!audioUrl) {
        reject(new Error(`Audio tidak tersedia untuk kata ${word}`));
        return;
      }
      ...
      try {
        ...
        audio.onerror = () => {
          setIsPlaying(false);
          setPlayingOption(null);
          reject(new Error(`Gagal memutar audio untuk kata ${word}`));
        };
  ...
  const pilihJawaban = async (option: string) => {
    ...
    if (option === currentQuiz.answer) {
      if (optionDetail?.audioUrl) {
        try {
          await playWordAudio(option, optionDetail.audioUrl);
        } catch (error) {
              ...
              return (
                ...
                >
                  <div className="flex flex-col items-center">
                    ...
                    <div className={`text-3xl font-bold mb-2 ${textClass}`}>
                      {option}
                    </div>
